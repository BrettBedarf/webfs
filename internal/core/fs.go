...
// LookupNodeID returns the Node for a given registry NodeID
func (fs *FileSystem) LookupNodeID(id uint64) (*Node, bool) {
	return fs.nodeRegistry.Load(id)
}
...
// GetChildCtx locks parent→child→inode (plus any FS locks you need),
// takes an Attr snapshot, and returns a NodeContext with its Close() wired up.
// Caller must do `defer ctx.Close()`.
func (fs *FileSystem) GetChildCtx(parentID uint64, name string) (ctx *NodeContext, ok bool) {
	parent, ok := fs.LookupNodeID(parentID)
	if !ok {
		return nil, false
	}
	parent.mu.RLock()
	...
	return &NodeContext{Node: child, closeFn: func() {
		child.mu.RUnlock()
		parent.mu.RUnlock()
	}}, true
}
