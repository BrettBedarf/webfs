...
func (r *FuseRaw) Lookup(cancel <-chan struct{}, header *fuse.InHeader, name string, out *fuse.EntryOut) fuse.Status {
	...
	parent, ok := r.fs.LookupNodeID(header.NodeId)
	if !ok {
		return fuse.ENOENT
	}
	// 2) lock the parent to prevent concurrent removal of child
	parent.mu.RLock()
	...
	// 3) allocate a new session-scoped NodeID
	newID := r.fs.AllocateNodeID(child)
	out.NodeId = newID
	// 4) copy attributes under the child's inode lock
	child.Inode.mu.RLock()
	attrCopy := *child.Inode.fuseAttr
	child.Inode.mu.RUnlock()
	out.Attr = attrCopy
	// 5) set TTLs
